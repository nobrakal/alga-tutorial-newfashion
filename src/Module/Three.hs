...
mod3 = Module
  ...
  subs =
    ...
    SubModule
      ...
      , fullAnswer = Answer "fmap (+ 1) gr" GraphInt "(==)" [("gr = 1 + 2 * 3 + 5 * 6",("Graph Int",True))]
    ...
    SubModule
      ...
      , fullAnswer = Answer "fmap (\\x -> if x == 0 then 1 else x) gr" GraphInt "(==)" [("gr = 1 + 2 * 3 + 5 * 6",("Graph Int",True))]
    ...
    SubModule
      ...
      , fullAnswer = Answer "sequence gr" CanFind "(==)" [("gr = Connect (pure (Just 1)) (pure (Just 2))",("Graph (Maybe Int)",True))]
    ...
    SubModule
      ...
      , fullAnswer = Answer "foldg 0 (const 1) (+) (+) gr" GraphInt "(==)" [("gr = 1 + 2 * 3 + 5 * 6",("Graph Int",True))]
    ...
    SubModule
      ...
      , fullAnswer = Answer "gr >>= (\\x -> overlay (vertex x) (vertex x))" GraphInt "(==)" [("gr = 1 + 2 * 3 + 5 * 6",("Graph Int",True))]
