...
mod4 = Module
  ...
  subs =
    ...
    SubModule
      ...
      , clue = "Use _induce_ "
      , fullAnswer = Answer "induce isPresent gr" GraphInt "(==)" ["gr = vertices [1,2,3,4,5,6,7,8,9]","isPresent x = (x==1)"]
    ...
    SubModule
      ...
      , fullAnswer = Answer "foldg (return empty) (\\x -> (\\y -> if y then vertex x else empty) <$> isPresent x) (liftA2 overlay) (liftA2 connect) gr" IOGraphInt "(==)" ["gr = vertices [1,2,3,4,5,6,7,8,9]","isPresent x = return (x==1)"]
      , conclusion = ""
      }
    ]
  }
