...
Every given example is runnable, so please feel free to install alga (with \verb|cabal| of \verb|stack|) and have a GHCi console near you if you want to try the code. All you need to have is contained in the \verb|Algebra.Graph| module.
\\
\\
If you encounter any bug (I hope that will not), please open an issue at \url{https://github.com/snowleopard/alga/issues/}
...
1 * (2 + 3) == 1 * 2 + 1 * 3
\end{lstlisting}
Here, connecting the singlet vertex 1 to both 2 and 3 can be done of two \emph{equivalent} ways: 
\begin{itemize}
...
This means that you have access to "lists-like" functions (like \verb|elem|, \verb|null|). It provide also a \verb|toList| function, but WARNING, this is not what you think ! It literally transform a graph into a list, so if you had a self-loop or any time two vertices, then you will have two times the same vertex.
